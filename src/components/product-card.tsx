...
function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div ref={wrapRef} className="group relative overflow-hidden bg-white p-7">
      <canvas ref={canvasRef} className="pointer-events-none absolute inset-0 z-0 h-full w-full" />
      <div className="relative z-10 flex flex-col">
        <p className="text-lg font-light tracking-wider text-[#1b1c1e]">{product.name}</p>
        <p className="mt-1 text-[11px] font-light tracking-[0.2em] text-[#9b9ea4]">{product.tag}</p>
        <p className="mt-5 text-xs font-light text-[#85888e]">{product.audience}</p>
        <p className="mt-4 text-sm font-light text-[#1b1c1e]">
          {product.price}
          <span className="ml-2 text-xs text-[#9b9ea4]">{product.period}</span>
        </p>
        <p className="mt-5 flex-1 text-xs font-light leading-6 text-[#85888e]">{product.note}</p>
      </div>
    </div>
  );
}
